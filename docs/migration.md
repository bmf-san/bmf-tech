...
1. Cloudflare Pages にプレビューデプロイして動作確認
2. Search Console で新 URL の sitemap を送信
3. DNS を Cloudflare に移管し `CNAME` を Cloudflare Pages に向ける
4. 旧 ConoHa VPS のトラフィックがゼロになったことを確認後、VPS を停止

### ロールバック方針

- 旧 VPS は DNS 切り替え後 1 ヶ月間維持し、問題があれば DNS を戻す
...
| 項目 | 状況 |
|---|---|
| サーバーサイド移行（gobel API・VPS） | 別途情報提供後に計画策定 |
