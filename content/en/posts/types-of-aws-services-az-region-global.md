---
title: Types of AWS Services - AZ, Region, Global
description: "Classify AWS services by deployment scope—Availability Zones, Regions, and Global services—with practical infrastructure examples."
slug: types-of-aws-services-az-region-global
date: 2021-06-23T00:00:00Z
author: bmf-san
categories:
  - Infrastructure
tags:
  - Amazon Web Service
translation_key: types-of-aws-services-az-region-global
---



# Overview
A note on the classification of representative AWS services.

# AZ Services
- Subnet (does not span multiple AZs)
- NAT
- Network ACL
- EC2 Instance
- RDS Instance
- Elastic Cache
- Elastic File System
- Elastic Load Balancing
- EBS Volume
- Redshift

# Region Services
- VPC (can span multiple AZs)
- Security Group
- VPC Endpoints
- VPC Peering
- Elastic IP
- Auto Scaling
- S3
- Glacier
- Snapshot (EBS/RDS)
- AMI
- DynamoDB
- SQS
- SNS
- CloudSearch
- CloudWatch
- Lambda
- API Gateway

# Global Services
- IAM (Users, Groups, Policies, Roles)
- Route53
- CloudFront
- CloudTrail
- WAF
- STS
