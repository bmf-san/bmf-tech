...
The measures that line up to address this are management practices: 1on1s, OKRs, review rules, metrics, and so on. Each one supports the quality of decisions, but if you keep adding them one by one, they easily become a loose collection whose overall aim you can no longer explain.
...
High quality alone leaves a decision closed inside one person. If the criteria are not put into words, delegation becomes "throwing it over the wall," and in the end you review everything yourself. That is exactly why quality needs to be paired with "reproducibility."

The state to aim for is "the quality of decisions is reproducible." Even when the person or the situation changes, the quality stays above a certain bar, and the decision can be explained. Once this axis is in place, many management practices line up as "parts for the quality and reproducibility of decisions."
...
The cards in the deck connect through four stages centered on "judgment." The further the stages advance, the more judgment moves away from a specific individual and toward a state the team can explain and reproduce. Each card (practice) also has its own purpose: it contributes to the deck's overall aim while, on its own, solving one particular problem.

As a concrete example, I list the cards of the deck I actually run, stage by stage. That said, these are only examples; there is no fixed set of cards you must have. Which cards you hold, in which stage, and how many, is decided to fit the state of the team—and as the team changes, the deck is reshuffled.
...
1. **Design where judgment sits**: clarify who owns which decision, and agree on the scope of delegation. For example, a "Delegation Board" makes visible on a single sheet which decisions are delegated and how far.
...
Laying them out by stage also reveals the dependencies between cards. For example, design principles only function once the metrics and materials that support them are in place, and a decision you made can only be reused once it is recorded in the decision journal. Each earlier stage supports the next.

# Building it isn't enough

This is the most important point: no practice works just by being built. Guidelines and checks, if they merely sit there, are read by no one. For them to turn into outcomes, there are conditions.

- **It keeps being operated**: if updating, recording, and running it stop, it quickly becomes a hollow formality.
- **It is actually used (adoption)**: it only works once it is referenced in the moments of judgment and review. If people forget, make it "work mechanically through a mechanism," like lint, CI, or AI review.
...
Whose what does this system improve? It can be organized in three layers.
...
- **The team**: "only-that-person-knows" decisions decrease, and the range of what can be delegated naturally widens. Judgment and knowledge do not stay in one person's memory; they accumulate on the team's side.
...
Finally, let me touch on the most easily misunderstood point. When I say I turn judgment into types, some people brace themselves: "won't that rob members of their individuality and constrain how they think?" In fact, it is rather the opposite.
...
- No practice works just by being built. It becomes an outcome only once it meets the conditions of operation, adoption, interlock, and agreement.
