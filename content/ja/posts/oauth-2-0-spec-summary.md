...
description: 'OAuth 2.0 を網羅的に解説。認可フロー・アクセストークン・リフレッシュトークン・スコープ、API セキュリティ実装のための実践的なノウハウを紹介します。'
slug: oauth-2-0
...
アプリケーションの種類によって適切なグラントタイプが異なる。OAuth 2.1以降ではインプリシットとパスワードグラントは削除されているため、以下を判断基準とする。

| アプリの種類 | 推奨グラントタイプ | 理由 |
|------------|-----------------|------|
| Webアプリ（サーバーサイドあり） | 認可コード + PKCE | クライアントシークレットをサーバーで安全に保管可能 |
| SPA（Single Page Application） | 認可コード + PKCE | インプリシットはOAuth 2.1で削除 |
| モバイルアプリ | 認可コード + PKCE | コードインターセプト攻撃をPKCEが防ぐ |
| サーバー間（M2M）通信 | クライアントクレデンシャル | ユーザーの介在が不要 |
| レガシーシステム移行 | 認可コード + PKCE | パスワードグラントはOAuth 2.1で削除 |

```mermaid
flowchart TD
    A[どのグラントタイプ？] --> B{ユーザーの操作が必要か？}
    B -->|No| C[クライアントクレデンシャル<br/>M2M通信・バッチ処理]
    B -->|Yes| D{クライアントがシークレットを<br/>安全に保持できるか？}
    D -->|Yes| E[認可コード + PKCE<br/>サーバーサイドアプリ]
    D -->|No| F[認可コード + PKCE<br/>SPA・モバイルアプリ]
```
