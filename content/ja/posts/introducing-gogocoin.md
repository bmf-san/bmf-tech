...
コードベースはレイヤー分離されたモジュラーアーキテクチャを採用している。`cmd/gogocoin`がエントリーポイント、`internal/`にドメインロジック・ユースケース・Infrastructureアダプター、`pkg/strategy`が戦略コントラクト定義と内蔵スキャルピング実装を提供する公開パッケージとなっている。
