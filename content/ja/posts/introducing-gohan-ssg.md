...
また、ほとんどのジェネレータは無条件全てのページを再生成するか、Git diff出力に依存するかのどちらかだ。Git diffはブランチ切り替えやフレッシュclone後に信頼性が落ちる。

Go製でシングルバイナリ、設定はシンプル、SHA-256マニフェストによる信頼性の高い差分ビルド——gohanはその要件を満たすために作った。
