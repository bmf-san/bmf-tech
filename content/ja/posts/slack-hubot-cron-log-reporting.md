...
description: hubot、node-cron、logwatch、CoffeeScriptを使い、サーバーログを定時にSlackへ報告する仕組みを自動化する方法を解説します。
...
* slack
* hubot
* hubot-slack3.4.2・・・hubotをslackで使うadapterです。hubotnのインストール時にadapterをslackに指定するとpackage.jsonに追加されて勝手にインストールされるようです。詳しくは~~さくらvps＋hubot＋Slackでslack botをつくる~~。ちなみにv2とv3系でchannel指定やAttachments利用可否が異なりますが、その辺は各自調べてください。。
* pm2・・・hubotのデーモン永続化。詳しくは~~さくらvps＋hubot＋Slackでslack botをつくる~~。
