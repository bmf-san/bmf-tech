...
移行後、サイトは安定かつ快適に運用できている。記事を書いてプッシュするだけでデプロイが完結し、サーバーの面倒を見る必要がない。振り返ると、もっと早くSSGという選択をすればよかったと感じている。ソースコードと記事は全て公開している。

今回の開発ではAIを積極的に活用した。ある程度の設計はしていたものの、時間が取れずに長らく放置していたプロジェクトだったが、AIの助けを借りてブログシステムの開発と移行をかなりのスピードで進めることができた。自作システムが小規模であることも要因のひとつだが、AIがなければ相当な工数がかかっていたはずだ。AIが登場するまで着手しなかったのは結果的に正解だったとさえ感じている。
