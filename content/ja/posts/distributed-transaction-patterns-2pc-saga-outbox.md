...
ほぼ無い。**マイクロサービス間では避けるのが定石**である。XA 等のトランザクションマネージャ配下で異種リソース（複数の DB、DB + MQ など）を確実に揃えたいレガシー寄りの場面で、限定的に検討する。
...
**COMMIT と publish のどちらを先にやっても、逆向きのズレが残る**。これが**二重書き込み（dual write）問題**である。
