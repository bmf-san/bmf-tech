...
description: "リレーショナルデータベースは継承をサポートしていないため、オブジェクト指向における継承関係をどのようにデータベース上へ表現するかを考える必要があります。この記事では、その表現方法である単一テーブル継承・クラステーブル継承・具象クラス継承という3つのパターンの特性と適用場面を、図解を交えて解説します。"
